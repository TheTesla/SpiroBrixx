...
def convert_params(params):
    ...
    dtp4 = float(par["dtp4"])
    name = f"spec_clamp_screwbarI_4_{l:02}_{w:02}_{h:02}"
...
def model_function(p):
    ...
    ang = np.atan2(y-rc-dwall,x-rc-dwall)
    rcc = ((y-rc-dwall)**2 + (x-rc-dwall)**2)**0.5
    flnk = rcc - rc + dwall * ang/np.pi + rbofase

    #a = bd.fz_cuboid((x-l*d/2,y-w*d/2,z-h*d/2), (l*d,w*d,h*d), rbofase)
    a = bd.fz_cuboid((x-l/2,y-w/2,z-h/2), (l,w,h), rbofase)
    b = bd.fz_circle((x-rc-dwall, y-rc-dwall), rc) +rbofase
    if cmb.fz_and_chamfer(rtifase, a, -b, y-x, flnk) >0: #-tx, -ty, -tz) > 0:
        return False
