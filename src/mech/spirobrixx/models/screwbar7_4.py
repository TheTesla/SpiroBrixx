...
def convert_params(params):
    ...
    dtp4 = float(par["dtp4"])
    name = f"screwbar7_4_{l:02}_{w:02}_{h:02}"
...
def model_function(p):
    ...
    tx = thrd.fz_thread((yr,zr,pt4*x-0.25), rt4i, 4, dtp4, 1.0) \
         if cutstep < 0 and cutstep > -2 and abs(x/l/d-0.5) > 0.5 - 1/l else rbofase
    ty = thrd.fz_thread((zr,xr,pt4*y-0.25), rt4i, 4, dtp4, 1.0) \
         if cutstep < 0 and cutstep > -2 and abs(y/w/d-0.5) > 0.5 - 1/w else rbofase
    tz = thrd.fz_thread((xr,yr,pt4*z-0.25), rt4i, 4, dtp4, 1.0) \
         if cutstep < 0 and cutstep > -2 else rbofase

    a = bd.fz_cuboid((x-l*d/2,y-w*d/2,z-h*d/2), (l*d,w*d,h*d), rbofase)
    screwbar = cmb.fz_and_chamfer(rtifase, a, -tx, -ty, -tz)
    if cmb.fz_and_chamfer(rbofase, screwbar, cut, -d*2-cut) > 0:
        return False
