...
ws += ls -5
d = draw.Drawing(210+7*ls+ws+xxp, 41, origin=(-95-8*ls/2-1,-21), displayInline=False)
clip = draw.ClipPath()
...
p = draw.Text('Spir', 40, -101-4*ls, 10, fill=c, style="font-family: osifont",
        letter_spacing=ls)
d.append(p)
p = draw.Text('Cli', 40, 18+ws, 10, fill=c, style="font-family: osifont",
        letter_spacing=ls)
d.append(p)

d.set_pixel_scale(2)

d.save_svg('SpiroClixxLogo.svg')
d.save_png('SpiroClixxLogo.png')
