...
def convert_params(params):
    ...
    dtp1 = float(par["dtp1"])
    dtp4 = float(par["dtp4"])
    name = f"screwbarI_4X1_{l:02}_{w:02}_{h:02}"
    return (rt1i, rt4i, pt1, pt4, d, rbofase, rtifase, l, w, h, dtp1, dtp4), name


@njit
def model_function(p):
    x, y, z, par = p
    rt1i, rt4i, pt1, pt4, d, rbofase, rtifase, l, w, h, dtp1, dtp4 = par

    xr = x % d - d/2
    yr = y % d - d/2
    zr = z % d - d/2

    tx = thrd.fz_thread((yr,zr,pt1*x-0.25), rt1i, 1, dtp1, 1.0)
