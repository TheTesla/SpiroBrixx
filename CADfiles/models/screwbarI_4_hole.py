...
def screwbarI_4_hole(p, par):
    ...
    rh = 5
    #ch = bd.fz_circle(((x+d/2)%d-d/2,(y+d/2)%d-d/2,z), rh)
    ch = bd.fz_circle((x-d,y-d,z), rh)

    a = bd.fz_cuboid((x-l*d/2,y-w*d/2,z-h*d/2), (l*d,w*d,h*d), re)
    if cmb.fz_and_chamfer(rge, a, -cx, -cy, -cz, -ch) > 0:
        return False
...
def new_screwbarI_4_hole(profile, parameters):
    ...
    name = f"screwbarI_4_hole_{l:02}_{w:02}_{h:02}"
    @njit
    def f(x, y, z):
        return screwbarI_4_hole((x,y,z), (rg, ra, pt4, d, re, rt4q, rge, l, w, h))
    return f, name
